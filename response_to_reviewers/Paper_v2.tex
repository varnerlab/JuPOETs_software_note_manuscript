...
JuPOETs integrates simulated annealing (SA) \cite{Kirkpatrick:1983aa} with Pareto optimality to estimate parameter sets on or near the optimal tradeoff surface between competing training objectives
(Fig. \ref{fig:fig-MOSAalgorithm} and Algorithm \ref{algo:JuPOETs-pcode}).
A tradeoff surface defines the best possible error value for every conflicting objective, and Pareto rank is a measure of distance away from the optimal tradeoff surface.
