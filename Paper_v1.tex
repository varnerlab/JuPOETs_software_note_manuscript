...
Ensemble modeling is a well established approach for dealing with parameter uncertainty in large-scale deterministic models.
...
\section*{Background}
Ensemble modeling is a well established approach for dealing with parameter uncertainty in large-scale deterministic models.
It is often not possible to uniquely identify biochemical model parameters from noisy experimental measurements, even when given extensive training data \cite{Gadkar:2005ad}.
...
In an ensemble approach, a family of parameter sets is identified instead of a single best-fit parameter set.
Parameter families can be selected based upon simulation error, along with other criterion such as diversity.
...
Identification of large biochemical models with hundreds or even thousands of states and parameters may not be tractable as a single objective formulation.
...
These diverse training data are likely heterogenous, and contain intrinsic conflicts which complicate parameter estimation.
On the other hand, large models can also be constructed from many smaller modules, each of which is independently identified.
...
lead to robust model performance. One class of such techniques is multiobjective optimization.
...
POETs, which integrates simulated annealing (SA) and multiobjective optimization through the notion of Pareto rank, finds a family of parameter estimates which optimally trade-off between
...
functions to accommodate custom cooling schedules, parameter constraints, or custom search logic. Octave-POETs was also not well integrated into a modern package or
source code management (SCM) system, thus upgrades containing new features, or bug fixes were not centrally managed.

\section*{Implementation}
In this study, we present an open-source implementation of the Pareto optimal ensemble technique in the Julia programming language (JuPOETs).
JuPOETs offers many advantages and improvements when compared to Octave-POETs.
...
\end{equation}using a modified simulated annealing algorithm.
The quantity t denotes time, $g_i\left(t,\mathbf{x}(t,\mathbf{p}),\mathbf{u}(t),\mathbf{p}\right)$ denotes the ith constraint on the search, where
...
The central idea underlying POETs is a mapping between the value of the objective functions evaluated at $\mathbf{p}_{i+1}$ (parameter guess at iteration $i+1$) and Pareto rank.
JuPOETs calculates the performance of a candidate parameter set $\mathbf{p}_{i+1}$ by calling the user defined objective function, \texttt{objective}. The function \texttt{objective} takes
a parameter set as an input argument and returns a $\mathcal{K}\times{1}$ error vector. Candidate parameter sets are generated by the user supplied \texttt{neighbor} function.
...
We implemented the Fonseca and Fleming ranking scheme in the \texttt{rank} function which computes the Pareto rank, $r$ \cite{RANKING}:
\begin{equation}\label{eqn_rank}
\texttt{rank}\left(\mathbf{p}_{i+1}\mid \mathcal{O}_{i}\right)=r
\end{equation} where rank is the number of parameter sets that dominate (are better than) parameter set $\mathbf{p}_{i+1}$.
...
\end{equation}
where $T$ is the computational annealing temperature. As $\texttt{rank}\left(\mathbf{p}_{i+1}\mid \mathcal{O}_{i}\right)\rightarrow{0}$, the acceptance probability moved toward one,
...
Parameter sets could be accepted by the SA and $\emph{not}$ archived in $\mathcal{S}_{i}$.
Only parameter sets with rank less than or equal to threshold (rank$\leq$4 by default) were included in $\mathcal{S}_{i}$
to ensure that we characterized the neighborhood near the trade-off surface.
...
\begin{equation}
  O_{i}+\lambda\sum_{j=1}^{\mathcal{C}}\min\left(0,g_j\left(t,\mathbf{x}(t,\mathbf{p}),\mathbf{u}(t),\mathbf{p}\right)\right)\qquad~i=1,\hdots,\mathcal{K}
...
JuPOETs can be installed using the Julia package manager, see instructions at the JuPOETs GitHub repository (https://github.com/varnerlab/POETs.jl).
...
If the new solution accepted (based upon a probability calculated with the user supplied acceptance function) it is added to the solution and objective archive. This solution is then perturbed during the next iteration of the algorithm. However, if the solution is not accepted, it is removed from the archive and discarded. The computational temperature is adjusted using the user supplied cooling function after each $\mathcal{I}$ iterations. }\label{algo:JuPOETs-pcode}
\end{algorithm}
...
(Fig. \ref{fig:fig-biochemical}A and B). The 95\% confidence estimate produced by the ensemble was consistent with mean of the measured data, despite having significant uncertainty in the data. JuPOETs produced a consensus estimate of the synthetic data by calculating optimal trade-offs between the synthetic data sets (Fig. \ref{fig:fig-biochemical}C).
Thus, JuPOETs produced an ensemble of parameter sets that gives the mean of the training data for conflicting data sets.

JuPOETs is a significant advancement over Octave-POETs, both in terms of performance and capability.
However, there are several areas that could be explored further.
...
Many evolutionary approaches e.g., the nondominated sorting genetic algorithm (NSGA) family of algorithms, have been adapted to solve multiobjective optimization problems [REF].
It is unclear if JuPOETs will perform as well as these other approaches;
...
Previously, this hybrid approach led to better convergence on a proof-of-concept signal transduction model [REF].
...
One strategy to improve performance could be to optimize surrogates [REF], while another would be parallel execution of the objective functions.
Currently, JuPOETs serially evaluate the components of the objective function vector.
However, because of the flexible function pointer architecture of JuPOETs, a shift to parallel evaluation of the objective functions requires changes to only the user defined objective function and not the main run loop. Thus, parallel evaluation of objective functions could be easily implemented using a variety of techniques without changing to JuPOETs.
