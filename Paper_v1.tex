...
POETs integrates simulated annealing (SA) with Pareto optimality to estimate parameter sets on or near the optimal tradeoff surface between competing training objectives
...
Parameter bounds constraints were implemented in the neighbor function while problem specific constraints were treated using the penalty method where each objective function was
augmented by the total of the constraint violation:
\begin{equation}
  O_{i}+\sum_{j=1}^{\mathcal{C}}\lambda_{j}\min\left(0,g_j\left(t,\mathbf{x}(t,\mathbf{p}),\mathbf{u}(t),\mathbf{p}\right)\right)\qquad~i=1,\hdots,\mathcal{K}
\end{equation}
